...
def chat_send_msg(msg):
    emit('chatoutput', {'data': '%s: %s' % (session['nick'], msg['data'])}, broadcast=True)
...
def chat_update_nick(msg):
    ...
    if nick in nicks.values():
        ...
        emit('chatoutput', {'data': 'Your nickname is either too short or too long.'})
        emit('nickoutput', {'data': session['nick']})
    elif " " in nick:
        ...
        #except KeyError:
        #    pass
        emit('chatoutput', {'data': '%s is now known as %s.' % (session['nick'], nick)},
             broadcast=True)
        session['nick'] = nick
        if game.players_by_uuid.has_key(session['nick']):
...
def chat_connect():
    uuid = session['uuid']
    if nicks.has_key(uuid):
        session['nick'] = nicks[uuid]
    if not session.has_key('nick'):
        num = len(nicks)
    ...
    emit('chatoutput', {'data': '%s joined the room.' % session['nick']}, broadcast=True)
    emit('nickoutput', {'data': session['nick']})
...
def game_connect():
    ...
    print('nicks: %s ' % nicks)
    emit('gameplayers', {'players': [(p.uuid, nicks[p.uuid]) for p in game.players ]}, broadcast=True)
    if game.started:
        emit('cards_update', {'hands': {p.uuid: p.group_successive() for p in game.players}}, broadcast=True)
        emit('cardup', {'card': game.cardup}, broadcast=True)
        emit('coinstatus', {'game': game.coins, 'players': { p.uuid: p.coins for p in game.players }}, broadcast=True)
        emit('scores', {'scores': {p.uuid: p.score for p in game.players}}, broadcast=True)
...
def chat_disconnect():
    ...
    #except KeyError:
    #    pass
    emit('chatoutput', {'data': '%s left the room.' % session['nick']}, broadcast=True)
    chat_update_nicklist()
...
def game_join():
    ...
    if not session['uuid'] in [ p.uuid for p in game.players ]:
        ...
        emit('gameoutput', {'text': '%s has joined the game.' % session['nick']},   broadcast=True)
        print('players uuid: %s' % ([p.uuid for p in game.players],))
        print(nicks)
        print('nicks: %s ' % nicks)
        emit('gameplayers', {'players': [(p.uuid, nicks[p.uuid]) for p in game.players ]}, broadcast=True)
...
def game_stop():
    if game.started:
        ...
        (winner, winner_points) = sorted(player_scores.items(), key=lambda i: i[1])[0]
        print('thewinneriz: %s' % winner)
        emit('gamewinner', {'winner': winner}, broadcast=True)
        emit('gameoutput', {'text': 'The winner is %s' % winner}, broadcast=True)
    else:
...
def game_pick(msg):
    ...
    if game.started:
        ...
        if current_player_uuid == session['uuid']:
            if action == 'pick':
                out = game.play(False)
                emit('cardpick', {'player': current_player_uuid, 'card':   current_card}, broadcast=True)
                emit('cardup', {'card': '%s' % game.cardup}, broadcast=True)
                emit('scores', {'scores': {p.uuid: p.score for p in game.players}}, broadcast=True)
            elif action == 'pass':
                if game.nextplayer.coins == 0:
                    emit('gameoutput', {'text': 'You shall not pass (no coins left).'})
                else:
                    out = game.play(True)
            emit('coinstatus', {'game': game.coins, 'players': { p.uuid: p.coins for p in game.players }}, broadcast=True)
            emit('gameoutput', {'text': '%s' % game}, broadcast=True)
...
def chat_update_nicklist():
    print(nicks)
    nicks2 = [ ('*%s' if n in [p.name for p in game.players] else '%s') % n for n in nicks.values()]
    print(nicks2)
    msg = "\n".join(nicks2)
    emit('nicklistupdate', {'data': msg}, broadcast=True)

def emit_game_infos(private=False):
    emit('gameinfo',   {'card':  '%s' % game.cardup,
                        'bonus': '%s' % game.coins,
                        'nextplayer': '%s' % game.nextplayer.name},
                        broadcast=True)
    if private:
        player = game.players_by_uuid[session['uuid']]
        emit('gameinfo_private',   {'score': '%s' % player.update_score(),
